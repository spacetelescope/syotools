...
# JT 9/10/2024
    ...
    def __init__(self, *arg, **kw):
        ...
        self.int_time = pre_encode(10. * u.hr)
        self.int_time = 10. * u.hr
        self.raw_contrast = 1e-10 # dimensionless
        self.eta_p = 0.2  # core throughput, taking the value for a type 1 coronagraph from Mennesson Figure 24
        self.Tr = 0.3 # total througput absent masks and detector QE - used number for all coronagraphs per Table 2
        self.quantum_eff = 0.9 # detector quantum efficiency (0.9 per Table 2)

        self.sigma_DeltaC = 5e-12 # the post-processing factor

        # all this below was needed only for calling the Ty code
        self.phase_angle = pre_encode(0. * u.deg)
        self.phase_func = pre_encode(1.)
        self.r_planet = pre_encode(1. * u.R_earth)
        self.semimajor = pre_encode(1. * u.au)
        self.t_eff = pre_encode(0. * u.K)
        self.r_star = pre_encode(1. * u.R_sun)
        self.d_system = pre_encode(0. * u.pc)
        self.n_exoz = 1.
        self.wave = pre_encode(np.zeros(0, dtype=float) * u.um)
        self.radiance = pre_encode(np.zeros(0, dtype=float) * (u.W / u.m**2 / u.um / u.sr))
        self.sol_flux = pre_encode(np.zeros(0, dtype=float) * (u.W / u.m**2 / u.um))
        #end of stuff for Ty code

        super().__init__(*arg, default_model=default_coronagraph, **kw)
